...
| 验收项（spec §7） | 目标 | 实测 | 备注 |
|---|---|---|---|
| 唤醒响应 | 说"你好小迪" 1s 内进对话窗口 + 提示音 | ____ |  |
| 多轮追问 | 对话窗口内连问 3 轮不重复唤醒 | ____ |  |
| barge-in | AI 说话时插嘴，TTS 300ms 内掐断→新录音 | ____ |  |
| 静默退出 | 10s 无活动回待机 | ____ |  |
| 唤醒命中率 | 10 次说"你好小迪" 10/10 命中 | ____ | 命中___/10 |
| barge-in 成功率 | 10 次插嘴 ≥8 次成功掐断 | ____ | ___/10 |
| 误唤醒（静音） | 1 小时静音 ≤2 次 | ____ | ___次 |
| 误唤醒（音乐） | 1 小时播音乐 ≤5 次 | ____ | ___次 |
| 按键模式切换 | 首页按钮切到按键模式，单击录音可用 | ____ |  |
| BYD 共存 | 喊"你好小迪"只触发本 app，不弹 BYD 界面 | ____ |  |
| 降级 | 模型加载失败→Toast+切按键模式，不崩 | ____ | 可改模型路径模拟 |
