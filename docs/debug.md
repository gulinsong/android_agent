...
**原因：** zai 插件的 base URL 硬编码为智谱云端 (`https://open.bigmodel.cn/api/paas/v4`)，需要改为局域网 llama.cpp 地址 (`http://172.20.10.2:8080/v1`)。
...
```bash
adb shell "sed -i 's|https://api.z.ai/api/coding/paas/v4|http://172.20.10.2:8080/v1|g; s|https://open.bigmodel.cn/api/coding/paas/v4|http://172.20.10.2:8080/v1|g; s|https://api.z.ai/api/paas/v4|http://172.20.10.2:8080/v1|g; s|https://open.bigmodel.cn/api/paas/v4|http://172.20.10.2:8080/v1|g' <文件路径>"
```
...
需要同时修改：
1. zai 插件源码中的 base URL → `http://172.20.10.2:8080/v1`
2. `models.json` 的 `baseUrl` → `http://172.20.10.2:8080/v1`
...
|------|-------------|-------------|
| `agents/main/agent/models.json` | `baseUrl: "https://open.bigmodel.cn/api/paas/v4"` | `baseUrl: "http://172.20.10.2:8080/v1"` |
...
以下服务运行在开发机（172.20.10.2, RTX 5000 16GB）上。
...
`models.providers.openai.baseUrl` 指向 `http://172.20.10.2:8090/v1`（STT 端口），不配 TTS 的 providers 就会发到错误的端口。
