...
| 上下文窗口 | O2.0/SC2.0 = **12K**（1594356 明确，app 当前用的版本）；Seeduplex 3.0 官方**未公开**，需控制台/方舟查 | deepseek-chat V3.1 = **128K**（V3 时代 API 限 64K，2025-08 起升 128K；V4-Flash 已 1M） |
...
| 方案 | 做法 | 判断 |
|---|---|---|
| **A. 全量替换（带 FC）** | Realtime/Seeduplex 取代整条 agent 语音链，工具定义迁移到火山控制台 | ✅ **可行**。拿到最低延迟 + 原生打断 + 省自建 STT/TTS；代价：工具定义迁控制台、LLM 换豆包、调试黑盒、需升级模型版本 |
...
- **端到端 Realtime 能驱动 agent**：FC 支持使其理论上可承接 skill/工具/卡片（需把工具定义迁到火山控制台 + 客户端处理 function_call 事件）。
- **现阶段仍建议 C（独立并存）**，理由：
